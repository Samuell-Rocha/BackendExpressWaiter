...
class CalcTotalItemOrderService {
  async execute({ order_id }: OrderRequest) {
    //const totals = await prisma.$queryRawUnsafe`SELECT amount FROM "OrderItens" WHERE id = ${order_id} `
    const calc =
      await prisma.$queryRaw<string>`SELECT SUM(amount * price) FROM "OrderItens" where id = ${order_id}`;
    ...
    const order = await prismaClient.orderItem.update({
      where: {
        id: order_id,
      },
      data: {
...
export { CalcTotalItemOrderService };
